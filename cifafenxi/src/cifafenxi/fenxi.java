	...
	public static String fenxiByLine(String string, HashMap<String, Integer> keyvalue)
	{
		...
		for(int i = 0;i < string.length();i++)
		{
			if (isLetter(string.charAt(i))) {
				...
				if (n == i+1) {
					returnString = returnString.concat("10 数字 "+string.charAt(i)+"\r\n");
				}
				else {
					String number = string.substring(i, n);
					returnString = returnString.concat("10 数字 "+number+"\r\n");
				}	
			}
				...
				if (!str.equals("") && (a = searchHashMap(str, keyvalue)) != 0) {
					...
					returnString = returnString.concat("9  变量"+str+"\r\n");
				}
				else {
					...
					if (judge == true) {
						judge = false;
						returnString = returnString.concat("10 数字 "+str+"\r\n");
					}
				}
				str = "";
			}
